...
export function safeLunchMonth(value: string | string[] | undefined): string {
...
export function LunchOverviewView({ overview, basePath = "/reditel", monthParam = "month", extraParams = {} }: {
  overview: LunchOverview; basePath?: string; monthParam?: string; extraParams?: Record<string, string>;
}) {
  const month = overview.month;
  const monthHref = (value: string) => {
    const params = new URLSearchParams(extraParams);
    params.set(monthParam, value);
    return `${basePath}?${params}#obedy`;
  };
  const monthLabel =
    overview.monthLabel.charAt(0).toUpperCase() + overview.monthLabel.slice(1);

  return (
    <section id="obedy" aria-labelledby="lunch-overview-title" className="min-w-0 scroll-mt-6 space-y-4">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <h2 id="lunch-overview-title" className="text-xl font-bold text-charcoal">Obědy</h2>
      ...
      <Card className="overflow-hidden p-0 ring-1 ring-charcoal/5">
        ...
        {overview.children.length === 0 ? (
          ...
          <div className="overflow-x-auto overscroll-x-contain max-md:max-h-[calc(100dvh-8rem)] max-md:overflow-y-auto max-md:overscroll-y-contain">
            <table aria-label={`Obědy – ${monthLabel}`} className="w-max min-w-full border-separate border-spacing-0 text-sm">
              <thead>
                <tr>
